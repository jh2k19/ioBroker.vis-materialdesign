...
function createMond(day) {
    ...
    if (lumiVal !== null && lumiVal !== undefined) {
...
function createCharts(day) {
    ...
    if (existsState(`${idDasWetterBase}.Hour_1.time`)) {
        // Prüfe wie viele Stunden verfügbar sind
        if (existsState(`${idDasWetterBase}.Hour_24.time`)) {
            maxHours = 24;
        } else if (existsState(`${idDasWetterBase}.Hour_8.time`)) {
            maxHours = 8;
        } else {
            // Zähle verfügbare Stunden
            for (let h = 1; h <= 24; h++) {
    ...
    if (maxHours === 0) {
        console.warn(`[createCharts] Day ${day} hat keine stündlichen Vorhersagedaten von DasWetter!`);
        // Erstelle leeren Chart
...
function createVorschauIconListItem(day) {
    ...
    // Symbol Description - prüfe welcher Datenpunkt gefüllt ist
    let symbolDesc = '';
    if (existsState(`${idDasWetter}.symbol_description`) && getState(`${idDasWetter}.symbol_description`).val) {
        symbolDesc = getState(`${idDasWetter}.symbol_description`).val;
    } else if (existsState(`${idDasWetter}.Weather_Symbol_Description`) && getState(`${idDasWetter}.Weather_Symbol_Description`).val) {
        symbolDesc = getState(`${idDasWetter}.Weather_Symbol_Description`).val;
    } else {
        symbolDesc = ''; // Leer lassen wenn nicht verfügbar
    }
    ...
    return {
        text: `<div style="margin: 0 4px; text-align: center;">${title}
            ...
            <div style="color: #44739e; font-family: RobotoCondensed-Regular; font-size: 16px; margin-top: 5px; text-align: center;">${getState(`${idDasWetter}.Temperature_Max`).val}°C &nbsp; | &nbsp; ${getState(`${idDasWetter}.Temperature_Min`).val}°C</div>
        ...
        image: getState(`${idDasWetter}.symbol_url`).val,
        subText: `<div style="display: flex; align-items: center; margin: 0 4px;">
            ...
            <div style="color: gray; font-family: RobotoCondensed-LightItalic; font-size: 10px;">${getState(`${idDasWetter}.Humidity`).val} %</div>
        </div>
        ...
        <div style="display: flex; align-items: center; margin: 0 4px;">
            ...
            <div style="color: gray; font-family: RobotoCondensed-LightItalic; font-size: 10px;">${getState(`${idDasWetter}.Wind_Speed`).val} km/h</div>
        </div>
        <div style="display: flex; align-items: center; margin: 0 4px;">
            <div style="flex: 1;text-align: left;font-family: RobotoCondensed-Light; font-size: 11px;">Luftdruck</div>
            <div style="color: gray; font-family: RobotoCondensed-LightItalic; font-size: 10px;">${getState(`${idDasWetter}.Pressure`).val} hPa</div>
        </div>
        <div style="display: flex; align-items: center; margin: 0 4px;">
            <div style="flex: 1;text-align: left;font-family: RobotoCondensed-Light; font-size: 11px;">Schneefall</div>
            <div style="color: gray; font-family: RobotoCondensed-LightItalic; font-size: 10px;">${getState(`${idDasWetter}.Snowline`).val} m</div>
        </div>
        <div style="display: flex; align-items: center; margin: 0 4px;">
